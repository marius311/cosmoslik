    ...
    Subroutine tetbeebbeb_lowl_likelihood(nlmaxin, clttin, cltein, cltbin, cleein, clbbin, clebin, chisq_r3, lndet)
        ...
        Do jp = 0, mp - 1
            Do ip = 0, jp
                ...
                !no idea why combining the last two lines fixes a compile error...
                NinvSNinv(mp+ip, mp+jp) = &
                    ...
                    + DDOT(nlmax-1, cleb(2:)-clte(2:)*cltb(2:)/cltt(2:), 1, Dble(ninvplninv(2:, np+ngood(ip), np+ngood(jp), 3)), 1) + DDOT(nlmax-1, cleb(2:)-cltb(2:)*clte(2:)/cltt(2:), 1, Dble(ninvplninv(2:, np+ngood(ip), np+ngood(jp), 4)), 1)
            End Do
