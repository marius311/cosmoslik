...
Contains
    Subroutine setup_for_tt_gibbs()
        ...
        If (gibbs_verbose) Then
            Print *, "------------------------------------------------------------"
            print *, "Reading from '"//trim(WMAP_data_dir)//trim(gibbs_sigma_filename)//"'"
        End If
        Call get_free_lun(lun)
        call read_gibbs_chain(trim(WMAP_data_dir)//trim(gibbs_sigma_filename), lun, ell_max, num_chains, num_samples, sigmas)
